...
export async function GET(_request: Request, { params }: IdContext) {
  try {
    ...
    if (!quote.pdf_bucket || !quote.pdf_path) {
      return jsonError("La cotización de ajuste no tiene PDF asociado.", 404);
    }
    ...
    if (downloadError || !file) {
      throw new Error(
        `Fallo al descargar el PDF de ajuste: ${downloadError?.message ?? "sin detalle"}`,
      );
    }

    const fileName = sanitizeHeaderFileName(
      quote.pdf_nombre_archivo ?? "cotizacion-ajuste-afisec.pdf",
    );
...
function sanitizeHeaderFileName(fileName: string) {
  return fileName.replace(/["\r\n]/g, "").trim() || "cotizacion-ajuste-afisec.pdf";
}
